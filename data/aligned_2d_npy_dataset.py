...
class Aligned2dNpyDataset(BaseDataset):
    ...
    # for aligned_2d_to_image: numpy array is of dimension (NC, NX, NY, NZ=1)
    # so we don't need to load everything in
...
#            transform_list.append(transforms.RandomHorizontalFlip())
    ...
    def __getitem__(self, index):
        ...
        osize = [self.opt.loadSize, self.opt.loadSize]
        zoom = (osize[0]/A.shape[0], osize[1]/A.shape[1], 1)
        A = scipy.ndimage.zoom(A, zoom=zoom, order=1)
        B = scipy.ndimage.zoom(B, zoom=zoom, order=1)
        ...
        A = A[i:(i+self.opt.fineSize), j:(j+self.opt.fineSize), :]
        B = B[i:(i+self.opt.fineSize), j:(j+self.opt.fineSize), :]

        if self.opt.isTrain and not self.opt.no_flip:
            r = random.random() * 360 - 180
            A = scipy.ndimage.rotate(A, angle=r, reshape=False, cval=0, order=1)
            B = scipy.ndimage.rotate(B, angle=r, reshape=False, cval=0.2, order=1)
#        B = scipy.ndimage.rotate(B, angle=r, reshape=False, cval=0, order=1)
