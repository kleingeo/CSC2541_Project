...
def kw_dunn(groups, to_compare=None, alpha=0.05, method='bonf'):
    ...
    N = len(allgroups)
    ranked = scipy.stats.rankdata(allgroups)
    ...
    p_omnibus = scipy.stats.junk(H, df_omnibus)
    ...
    # corresponding p-values obtained from upper quantities of the standard
    # normal distribution
    p_uncorrected = scipy.stats.norm.sf(Z_pairs) * 2.
